...
import edu.columbia.dbmi.cwlab.pojo.CdmCriterion;
...
public class IOUtil {
	public static String doc2Str(Document doc) {
		...
		String inc_entities = Pargraph2List(incps, "INC","NCTID");
		String exc_entities = Pargraph2List(excps, "EXC","NCTID");

		return inc_entities + exc_entities;
	}

	public static String Pargraph2List(List<Paragraph> incps, String inctag,String nctid) {
		StringBuffer sb = new StringBuffer();
		if (incps != null) {
			for (int k=0;k<incps.size();k++) {
				List<Sentence> sents = incps.get(k).getSents();
				if (sents != null) {
					for (Sentence s : sents) {
						List<Term> terms = s.getTerms();
						List<Triple<Integer, Integer, String>> relations = s.getRelations();
						for (Term t : terms) {
							if (Arrays.asList(GlobalSetting.allclasses).contains(t.getCategorey())) {
								...
								sb.append(nctid+"\t"+inctag + "\t" +k+"\t" + t.getText() + "\t" + t.getCategorey() + "\t" + t.isNeg()
								+"\t"+t.getStart_index()+"\t"+t.getEnd_index()+ "\t" + temporalstr + "\t" + valuestr + "\n");
							}
